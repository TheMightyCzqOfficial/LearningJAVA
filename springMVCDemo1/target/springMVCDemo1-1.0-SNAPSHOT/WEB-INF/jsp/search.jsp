...
<div class="container" style="height: 100%;width: 100%">
  ...
  <div style="text-align: center;margin: 5px">
    ...
    <a  class="btn btn-default" href="${pageContext.request.contextPath}/stockList?page=1"  >返回</a>
  </div>
  ...
  <nav aria-label="Page navigation">
    <ul class="pagination  pagination-lg">
      <li>
        <a href="${pageContext.request.contextPath}/search?name=${param.name}&page=1&size=${pageInfo.pageSize}" aria-label="Previous">
          <span aria-hidden="true">首页</span>
        </a>
        <a href="${pageContext.request.contextPath}/search?name=${param.name}&page=${pageInfo.pageNum-1}&size=${pageInfo.pageSize}" >
        ...
        <c:if test="${pageInfo.pages<10}">
          <c:forEach begin="1" end="${pageInfo.pages}" var="num">
            <c:if test="${pageInfo.pageNum==num}">
              <li class="active"><a href="${pageContext.request.contextPath}/search?name=${param.name}&page=${num}&size=${pageInfo.pageSize}">${num}</a></li>
            </c:if>
            <c:if test="${pageInfo.pageNum!=num}">
              <li ><a href="${pageContext.request.contextPath}/search?name=${param.name}&page=${num}&size=${pageInfo.pageSize}">${num}</a></li>
            </c:if>
          </c:forEach>
        </c:if>
      ...
      <c:if test="${pageInfo.pageNum>=pageInfo.pages-4 && pageInfo.pages>5 && pageInfo.pageNum>5}">
      ...
      <li>
        <a href="${pageContext.request.contextPath}/search?name=${param.name}&page=${pageInfo.pageNum+1}&size=${pageInfo.pageSize}" aria-label="Next">
          <span aria-hidden="true">&raquo;</span>
        </a>
        <a href="${pageContext.request.contextPath}/search?name=${param.name}&page=${pageInfo.pages}&size=${pageInfo.pageSize}" >
