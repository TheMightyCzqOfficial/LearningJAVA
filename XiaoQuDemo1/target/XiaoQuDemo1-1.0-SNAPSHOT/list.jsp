...
<div class="container" style="height: 100%;width: 100%">
    ...
    <div style="float: right;margin: 5px">
        ...
        <a class="btn btn-primary" href="${pageContext.request.contextPath}/UnregisterServlet">添加业主</a>
    ...
    <table border="1" class="table table-bordered table-hover">
       ...
       <c:forEach items="${pageBean.list}" var="user" varStatus="s">
        <tr>
            ...
            <td>${user.house}</td>
            <td>${user.username}</td>
            <td><a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/findUserServlet?id=${user.id}" style="color: greenyellow">修改</a>&nbsp;
                <a class="btn btn-default btn-sm" href="javascript:deleteUser('${user.id}','${user.name}')" style="color: red">删除</a></td>
        </tr>
       </c:forEach>
    ...
    <c:if test="${unDeal!=null}">
        <div class="alert alert-warning" role="alert"><strong>提示:</strong> ${unDeal}</div>

    </c:if>
    <c:if test="${unDo!=null}">
        <div class="alert alert-danger" role="alert"><strong>提示: </strong> ${unDo}</div>
